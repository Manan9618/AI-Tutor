...
async def get_recommendations(current_user: str = Depends(get_current_user)):
    ...
    """
    try:
        # ✅ ADD 'await' here to actually execute the async function
        recommendations = await analytics_agent.generate_recommendations(current_user, memory_agent)
        return {"recommendations": recommendations}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error generating recommendations: {str(e)}")
...
async def get_dashboard(current_user: str = Depends(get_current_user)):
    try:
        # ✅ ADD 'await' here as well if generate_dashboard_metrics is async
        metrics = await analytics_agent.generate_dashboard_metrics(current_user, memory_agent)
...
async def update_analytics(data: dict, current_user: str = Depends(get_current_user)):
    ...
    try:
        ...
        # Always recompute dashboard
        dashboard = await analytics_agent.generate_dashboard_metrics(current_user, memory_agent)
        return {"status": "success", "analytics": dashboard}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))