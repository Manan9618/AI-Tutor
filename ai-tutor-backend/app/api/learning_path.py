...
# from app.api import path_generator, memory_agent, analytics_agent
...
#     try:
#         next_topic = path_generator.generate_next_topic(current_user, memory_agent)
...
#     """
#     try:
#         # 1️⃣ Try to load saved path
#         saved_path = memory_agent.get_user_learning_path(current_user)

#         if saved_path:
#             return {"topics": saved_path}

#         # 2️⃣ Otherwise, generate a new one
#         roadmap = path_generator.generate_roadmap(current_user, memory_agent)
#         topics = [{"id": i + 1, "name": t, "completed": False} for i, t in enumerate(roadmap)]

#         # 3️⃣ Save in memory for persistence
#         memory_agent.save_learning_path(current_user, topics)
...
#         # Record completion progress to analytics
#         completed = sum(1 for t in topics if t.get("completed"))
#         total = len(topics)
#         progress = round((completed / total) * 100, 2) if total > 0 else 0
#         analytics_agent.record_learning_progress(current_user, completed, total, progress)

#         return {"message": "✅ Learning path updated", "progress": progress}
